...
function keysToSnakeCase(obj) {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    snakeCaseObj[snakeCaseKey] = keysToSnakeCase(obj[key]);
  });
...
export function expandResourceData(data, userData) {
  ...
  function addFieldsToData(gData, fields, replyNameToChosenUserData) {
    return gData.map((item) => {
      const nameKey = item.name.toLowerCase(); // name is expected in 'Surname, GivenName' format
      const userDataByKey = replyNameToChosenUserData[nameKey];
      ...
      if (userDataByKey) {
        fields.forEach((field) => {
          ...
          item[camelCaseField] = userDataByKey[field];
        });
      } else {
  ...
  function addFieldsToMissing(chosenData, fieldsToAdd2, replyDiff2) {
    return chosenData.map((item) => {
      // Check if faxNumber is not null and not undefined
      const nameKey = item.name.toLowerCase(); // name is expected in 'Surname, GivenName' format

      if (nameKey && replyDiff2[nameKey]) {
        const userDataI = replyDiff2[nameKey];
        fieldsToAdd2.forEach((field) => {
          // Convert snake_case to camelCase for the field name
          const camelCaseField = field.replace(/_([a-z])/g, (match, p1) =>
            p1.toUpperCase()
          );
          // eslint-disable-next-line no-param-reassign
          item[camelCaseField] = userDataI[field];
        });
      }
      return item;
    });
  ...
  const groupedMonthData = augmentedData.map((item) => {
    ...
    item.jobs.forEach((job) => {
      ...
      if (job.exp_coge === 'Y') {
        ...
        daysAllocated = parseInt(job.days, 10) || 0; // Ensure days is a number, default to 0 if NaN
      } else {
        // this is not forcast yet
        daysForecast = parseInt(job.days, 10) || 0; // Ensure days is a number, default to 0 if NaN
      }
      ...
      if (
        ...
        (typeof job.hypo_dd === 'string' && Number(job.hypo_dd))
      ) {
      ...
      if (
        ...
        (typeof job.work_dd === 'string' && Number(job.work_dd))
      ) {
      ...
      // Sum the days
      jobsByMonth[month].days_allocated += daysAllocated;
      jobsByMonth[month].days_forecast += daysForecast;
    });
