...
const UsersDialog = ({ open, onClose, resourcingData, resources }) => {
  ...
  useEffect(() => {
    ...
    const uniqueDepartments = Array.from(
      new Set(resources.map((user) => user.department).filter(Boolean))
    );
  ...
  const onAddUser = async (user) => {
    const recordProposal = {
      resource: user.mail,
      role_id: resourcingData.role_id,
      displayName: user.displayName,
      customer: resourcingData.Customer,
      project: resourcingData.Description,
      code: resourcingData.Code,
      role: resourcingData.role,
      monthlyDetails: resourcingData.monthlyDetails,
    };
  ...
  const processData = (chosenView, users) => {
